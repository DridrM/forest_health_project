...
def show_training_images(
    ...
    """
    Display a grid of training images using Matplotlib.

    This function takes a batch of training images (which are single-channel grayscale images)
    and arranges them in a grid. It uses `torchvision.utils.make_grid()` to create the grid and
    `matplotlib.pyplot` to display the images. Since training images are grayscale, the function
    ensures that only the first channel is used for display.
    ...
    plt.imshow(image_grid[0, :, :])  # Choose first channel for B&W images
    plt.axis("off")
    plt.show()
