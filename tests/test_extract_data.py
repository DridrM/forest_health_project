...
from fhealth.dataset.extract_data import DataStatus, GCPDataHandler


# Fixture to initialize GCPDataHandler with mocked GCP components
@pytest.fixture
def mock_gcp_data_handler(monkeypatch):
    """
    Fixture that initializes a GCPDataHandler instance with mocked GCP components.
    """
    ...
    # Initialize GCPDataHandler
    handler = GCPDataHandler(
...
def test_init(mock_gcp_data_handler):
    """
    Test GCPDataHandler initialization with mock credentials and client.
    """
    assert mock_gcp_data_handler.client is not None
    assert mock_gcp_data_handler.bucket is not None


def test_load_rgb_image(monkeypatch, mock_gcp_data_handler):
    """
    ...
    mock_gcp_data_handler.load_rgb_image("fake_image_blob")

    # Assert the image was loaded into the handler as a PIL image
    assert isinstance(mock_gcp_data_handler.rgb_image, Image.Image)


def test_load_mask(monkeypatch, mock_gcp_data_handler):
    """
    ...
    monkeypatch.setattr(GCPDataHandler, "download_blob", lambda *args: fake_pickle_data)

    # Call the method to load the pickle object
    mock_gcp_data_handler.load_mask("fake_pickle_blob")

    # Assert the mask was correctly loaded into the handler
    assert isinstance(mock_gcp_data_handler.mask, list)
    assert isinstance(mock_gcp_data_handler.mask[0], Polygon)


def test_set_data_status(mock_gcp_data_handler):
    """
    Test the set_data_status method for updating the data status field.
    """
    # Set and verify data status to 'train'
    mock_gcp_data_handler.set_data_status(DataStatus.train)
    assert mock_gcp_data_handler.data_status == DataStatus.train

    # Set and verify data status to 'valid'
    mock_gcp_data_handler.set_data_status(DataStatus.valid)
    assert mock_gcp_data_handler.data_status == DataStatus.valid

    # Set and verify data status to 'test'
    mock_gcp_data_handler.set_data_status(DataStatus.test)
    assert mock_gcp_data_handler.data_status == DataStatus.test


def test_download_blob(monkeypatch, mock_gcp_data_handler):
    """
    ...
    monkeypatch.setattr(
        mock_gcp_data_handler.bucket, "blob", lambda blob_name: mock_blob
    )

    # Call the download_blob method
    blob_data = mock_gcp_data_handler.download_blob("fake_blob")
